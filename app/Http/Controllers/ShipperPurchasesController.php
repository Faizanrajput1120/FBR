...
class ShipperPurchasesController extends Controller
{
    public function index()
    {
        ...
        if ($erpParams->isNotEmpty()) {
            ...
            $accountSuppliers = AccountMaster::whereIn('level2_id', [4, 7,23])->get();

            $purchaseAccountId = $erpParams->first()->purchase_account;
            $purchaseAccount = AccountMaster::find($purchaseAccountId);
        }
        return view('shipper_purchases.list',get_defined_vars());
    }
   public function store(Request $request)
{
    $maxVoucherNo = ShipperPurchases::max('vorcher_no');
$newInvoiceNumber = $maxVoucherNo ? ((int) $maxVoucherNo + 1) : 1;
    ...
    ->value('item_code');
        ...
        $purchaseGlue = ShipperPurchases::create([
   ...
   $Purfreight = $erpParam ? $erpParam->pur_freight : null;
    ...
    return redirect()->route('shipper_purchases.reports')->with('success', 'Voucher No. ' . $newInvoiceNumber . ' has been saved successfully.');
}
    ...
    $account_id = $request->input('account_id');
        ...
        $query = TRNDTL::where('v_type', 'SPN')->where('debit', 0)->where('account_id', '!=', 35)->with('shipperpurchases');
   ...
   // Fix incorrect method chaining
    $vNo = TRNDTL::where('v_type', 'SPN')->pluck('v_no')->unique()->toArray();
    $accountId = AccountMaster::whereIn('id', TRNDTL::where('v_type', 'SPN')->pluck('account_id'))
    ->where('title', '!=', 'Purchase Freight') // Exclude "Purchase Freight"
    ->pluck('title', 'id');
    
        return view('purchase_reports.index9', [
            'trndtl' => $trndtl,
            'startDate' => $startDate,
            'endDate' => $endDate,
  ...
  public function edit($v_no)
{
    ...
    return view('purchase_reports.edit9', compact('v_no', 'loggedInUser', 'voucher', 'erpParams', 'accountMasters', 'accountSuppliers', 'purchaseAccount', 'items'));
}
...
public function update(Request $request, $id)
{
    ...
    try {
        foreach ($request->entries as $entry) {
            ...
            $shipperPurchases = ShipperPurchases::create([
                'item_code' => $entry['item'] ?? null,
                'qty' => $entry['quantity'] ?? 0,
                'rate' => $entry['rate'] ?? 0,
                'amount' => $entry['amount'] ?? 0,
            ...
            TRNDTL::create([
                ...
                'debit' => 0,
                'status' => 'unofficial',
                'description' => ($itemCode ? $itemCode : '') . ' x ' .
                                 ($entry['quantity'] ?? 0) . ' Rolls @ ' .
                                 ($entry['rate'] ?? 0),
            ]);
...
public function destroy($id)
{
    ...
    if ($trndtl->v_type === 'SPN') {
        // Delete all related TRNDTL records where r_id matches
        TRNDTL::where('r_id', $trndtl->r_id)->where('v_type', 'SPN')->delete();

        // Delete the related ShipperPurchases record if it exists
        $purchaseDetail = ShipperPurchases::find($trndtl->r_id);
        if ($purchaseDetail) {
...
public function editBoxboard($v_no)
{
    ...
    $purchaseDetail = ShipperPurchases::where('vorcher_no', $v_no)->first();
    $freight_type = $purchaseDetail ? $purchaseDetail->freight_type : null;

    // Sum the qty from purchase_details for the same voucher_no
    $totalQty = ShipperPurchases::where('vorcher_no', $v_no)->sum('qty');

    // Pass $freight, $v_no, $totalQty, and $freight_type to the view
    return view('purchase_reports.editShipper', compact('freight', 'v_no', 'totalQty', 'freight_type'));
}
public function updateBoxboard(Request $request, $id)
{
    ...
    try {
        ...
        $existingPurchaseDetail = ShipperPurchases::where('vorcher_no', $id)->first();
        ...
        if ($validatedData['total_freight'] > 0) {
            ...
            if ($existingPurchaseDetail) {
                ...
                $purchaseDetail = ShipperPurchases::create([
                    'vorcher_no' => $id,
                    'freight' => $validatedData['total_freight'],
                    'freight_type' => $validatedData['freight_type'], // Add this line
       ...
       return redirect()->route('shipper_purchases.reports')->with('success', 'Freight updated successfully for SPN-' . $id);

    } catch (\Exception $e) {
        // Handle any exceptions and return an error message
        return redirect()->route('shipper_purchases.reports')->with('error', 'An error occurred: ' . $e->getMessage());
    }
}
