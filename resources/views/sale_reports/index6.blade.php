...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h3 class="page-title">Pharmaceutical Billing</h3>
            </div>
        </div>
    </div>
    <!-- end page title -->
        ...
        <div class="card mt-2">
            <div class="card-body">
                <div class="tab-content">
                    <div class="col-12">
                        <form action="{{ route('pharma_billing.reports') }}" method="GET" class="form-inline"
                            id="search-form">
                            <div class="row">
                                <!-- Start Date -->
                             ...
                             <div class="form-group col-xl-2">
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    <a class="btn btn-success" href="{{ route('pharma_billing.list') }}"
    ...
    @foreach($trnDetails as $data1)
        @php
            ...
            $uniqueItemTitles = []; // Array to keep track of unique item titles
            $itemTitle = 'N/A'; // Initialize itemTitle outside the loop to handle cases where it's not found
        @endphp
        @foreach($saleInvoicesForVNo as $saleInvoice)
            ...
            <!-- Only add to the list and display the row if itemTitle is not 'N/A' -->
            @if($itemTitle !== 'N/A' && !in_array($itemTitle, $uniqueItemTitles))
                @php
                    $uniqueItemTitles[] = $itemTitle; // Add the item title to the unique list
                @endphp
            @endif
        @endforeach

        <!-- Only display the row if at least one itemTitle is not 'N/A' -->
        @if(count($uniqueItemTitles) > 0)
            <tr>
                <td>{{ \Carbon\Carbon::parse($data1->date)->format('d-m-Y') ?? 'N/A' }}</td>
               <td>{{ strtoupper(substr($data1->accounts->title ?? 'N/A', 0, 1)) . strtoupper(substr(explode(' ', $data1->accounts->title ?? 'N/A')[1] ?? '', 0, 1)) }}-{{ $data1->v_no }}</td>
                <td>{{ $data1->accounts->title ?? 'N/A' }}</td>
                <td>{{ implode(', ', $uniqueItemTitles) }}</td>
                <td>{{ number_format($data1->debit ?? 'N/A') }}</td>
                <td>
                    <form action="{{ route('pharma_billing.destroy',['billing_no' => $data1->r_id]) }}" 
      method="POST" 
...
<div style="display: flex; justify-content: space-between; align-items: center; margin: 10px 0;">
    <h4 id="print-header1" style="margin: 0; display: none;" >Date: {{ \Carbon\Carbon::parse($data1->created_at)->format('d-m-Y') ?? 'N/A' }}</h4>
    @if($vNoList)
...
@endforeach
    ...
    @php
        ...
        // Filter invoices based on selected account and sort by old_vno
        $filteredInvoices = $selectedAccountId 
            ? $saleInvoices->where('account_id', $selectedAccountId)->sortBy('old_vno')->values()
            : $saleInvoices->sortBy('old_vno')->values();
    @endphp
    ...
    @foreach($filteredInvoices as $index => $data)
        ...
        {{-- Check if the v_no changes or this is the last iteration --}}
        @if($gt !== 'N/A' && ($index === $filteredInvoices->count() - 1 || $filteredInvoices[$index + 1]->v_no !== $data->v_no))
        <tr>
 ...
 function checkPermission() {
        ...
        if ($isAdmin == 0) {
            $userRights = \App\Models\Right::where('user_id', auth()->user()->id)
                ->where('app_name', 'pharmaceuticalbilling')
                ->first();
