    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h3 class="page-title">Plate Purchase</h3>
            </div>
        </div>
    </div>
    <!-- end page title -->
...
@endif
    ...
    <div class="row">
        ...
        <div class="col-6">
            <form action="{{ route('plate_purchase.reports') }}" method="GET" class="form-inline" id="search-form">
                <div class="row">
    ...
    <select name="account_id" class="form-control select2" data-toggle="select2">
        ...
        @endforeach
                ...
                <div class="form-group mt-3">
                                    ...
                                    <a class="btn btn-success" href="{{ route('plate_purchase.list') }}" role="button" onclick="return checkPermission()">Add New</a>
                                </div>
                </div>
            </form>
        </div>
            ...
            <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $data->purchaseplates->items->item_code ?? 'N/A'}}</td>
                        <td>{{ $data->purchaseplates->countries->country_name ?? 'N/A'}}</td>
                        <td>{{ $data->purchaseplates->products->prod_name ?? 'N/A'}}</td>
                        <td>{{ $data->purchaseplates->description }}</td>
                        <td>{{ $data->purchaseplates->qty }}</td>
                        <td>{{ $data->purchaseplates->rate }}</td>
                        <td>{{ $data->purchaseplates->amount }}</td>
                        <td class="no-print">
                                                <input type="checkbox"
                        ...
                        <td class="no-print">
                            <form action="{{ route('plate_purchase.delete', ['id' => $data->id]) }}" method="POST" style="display:inline;" onclick="return checkPermissionDel()">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete(this)">Delete</button>
                            </form>

                            <a href="{{ route('plate_purchase.edit', ['v_no' => $data->v_no]) }}" class="btn btn-warning btn-sm" onclick="return checkPermissionEdit()">Edit</a>
                        </td>
...
function printTable() {
    ...
    @php
        $freightValues = $trndtl->pluck('purchaseplates.freight')->filter(function ($value) {
