                    ...
                    <div class="card-body">
                        <div class="tab-content">
                            <div class="tab-pane show active" id="input-types-preview">
                                <div class="row">
                                    <form id="voucherForm"
                                        action="{{ route('corrugation_purchase.update', $voucher->first()->v_no) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="col-6">
                                                    ...
                                                    @if ($voucher->isNotEmpty())
                                                        @foreach ($voucher as $trndtl)
                                                            <tr>
                                                                ...
                                                                <!-- Account title -->
                                                                   <td>{{ $trndtl->corrugationpurchases->item->item_code ?? 'N/A' }}</td>
                                                                <td>{{ $trndtl->corrugationpurchases->qty }}</td>
                                                                <td>{{ $trndtl->corrugationpurchases->size }}</td>
                                                                
                                                                <td>{{ $trndtl->corrugationpurchases->rate }}</td>
                                                                <td>{{ $trndtl->corrugationpurchases->amount }}</td>
                                                                <td>{{ $trndtl->corrugationpurchases->freight }}</td>
                                                                <td>
                                                                    <!-- Delete Entry Button -->
                                                                    <a href="{{ route('corrugation_purchase.destroy', $trndtl->id) }}"
                                                                        class="btn btn-danger btn-sm"
                                                                        onclick="event.preventDefault();
                                                                            if(confirm('Are you sure you want to delete this transaction?')) {
                                                                                window.location.href='{{ route('corrugation_purchase.destroy', $trndtl->id) }}';
                                                                            }">Delete</a>
                                                                </td>
                                                            </tr>
                                                        @endforeach
