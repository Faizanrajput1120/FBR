            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Shipper Purchase</li>
                    </ol>
                </div>
                <h3 class="page-title">Shipper Purchase</h3>
            </div>
        </div>
    </div>
    <!-- end page title -->
...
@endif
    ...
    <div class="row">
        ...
        <div class="col-6">
            <form action="{{ route('shipper_purchases.reports') }}" method="GET" class="form-inline" id="search-form">
                <div class="row">
    ...
    <select name="account_id" class="form-control select2" data-toggle="select2">
        ...
        @endforeach
               ...
               <div class="form-group mt-3">
                                    ...
                                    <a class="btn btn-success" href="{{ route('shipper_purchases.list') }}" role="button" onclick="return checkPermission()">Add New</a>
                                </div>
                </div>
            </form>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-12">
            ...
            <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                  ...
                  <h4>Shipper Purchase Details</h4>
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $data->shipperpurchases->items->item_code ?? 'N/A'}}</td>
                        <td>{{ $data->shipperpurchases->qty ?? 'N/A'}}</td>
                        <td>{{ $data->shipperpurchases->rate ?? 'N/A'}}</td>
                        <td>{{ $data->shipperpurchases->amount ?? 'N/A'}}</td>
                        <td class="no-print">{{ $data->shipperpurchases->freight ?? 'N/A'}}</td>
                        <td class="no-print">
                                                <input type="checkbox"
                        ...
                        <td class="no-print">
                            <form action="{{ route('shipper_purchases.delete', ['id' => $data->id]) }}" method="POST" style="display:inline;" onclick="return checkPermissionDel()">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete(this)">Delete</button>
                            </form>

                            <a href="{{ route('shipper_purchases.edit', ['v_no' => $data->v_no]) }}" class="btn btn-warning btn-sm " onclick="return checkPermissionEdit()">Edit</a>
                      <a href="{{ route('shipper_purchases.editBoxboard', ['v_no' => $data->v_no]) }}" class="btn btn-primary btn-sm mt-1">Freight</a>
 ...
 function checkPermission() {
        ...
        if ($isAdmin == 0) {
            $userRights = \App\Models\Right::where('user_id', auth()->user()->id)
                ->where('app_name', 'ShipperPurchase')
                ->first();
    ...
    @php
        ...
        if ($isAdmin == 0) {
            $userRights = \App\Models\Right::where('user_id', auth()->user()->id)
                ->where('app_name', 'ShipperPurchase')
                ->first();
            $canDelete = $userRights && $userRights->del == 1;
        }
    @endphp
...
const freightContent = `
   @php
    ...
    $freightDetails = $trndtl->pluck('shipperpurchases')->filter(function ($detail) {
