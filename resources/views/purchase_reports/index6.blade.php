        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h3 class="page-title">Corrugation Purchase</h3>
                </div>
            </div>
        </div>
        <!-- end page title -->
            ...
            <div class="card mt-2">
                <div class="card-body">
                    ...
                    <div class="tab-content">
                        <div class="col-6">
                            <form action="{{ route('corrugation_purchase.reports') }}" method="GET" class="form-inline"
                                id="search-form">
                                <div class="row">
    ...
    <select name="account_id" class="form-control select2" data-toggle="select2">
        ...
        @endforeach
                                    ...
                                    <div class="form-group mt-3">
                                        ...
                                        <a class="btn btn-success" href="{{ route('corrugation_purchase.list') }}"
                            ...
                            <div class="tab-content">
                                <div class="col-12">
                                    ...
                                    <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                                        ...
                                        <h4>Corrugation Purchase Details</h4>
                                        <thead>
                                            <tr>
...
<td>{{ $data->corrugationpurchases->item->item_code ?? 'N/A' }}</td>

                                                    <td>{{ $data->corrugationpurchases->qty ?? 'N/A' }}</td>
                                                    <td>{{ $data->corrugationpurchases->rate ?? 'N/A' }}</td>
                                                    <td>{{ $data->corrugationpurchases->size ?? 'N/A' }}</td>
                                                    <td>{{ $data->corrugationpurchases->amount ?? 'N/A' }}</td>
                                                    <td class="no-print">{{ $data->corrugationpurchases->freight ?? 'N/A' }}</td>
                                                    <td class="no-print">
                                                    ...
                                                    <td class="no-print">
                                                        <form
                                                            action="{{ route('corrugation_purchase.delete', ['id' => $data->id]) }}"
                                                        ...
                                                        <a href="{{ route('corrugation_purchase.edit', ['v_no' => $data->v_no]) }}"
                                                            class="btn btn-warning btn-sm" onclick="return checkPermissionEdit()">Edit</a>
                                                            
                                                               <a href="{{ route('corrugation_purchase.editBoxboard', ['v_no' => $data->v_no]) }}" class="btn btn-primary btn-sm mt-1" >Freight</a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
    ...
    function checkPermission() {
        ...
        if ($isAdmin == 0) {
            $userRights = \App\Models\Right::where('user_id', auth()->user()->id)
                ->where('app_name', 'CorrugationPurchase')
                ->first();
...
function printTable() {
   ...
   const freightContent = `
    @php
        $freightDetails = $trndtl->pluck('corrugationpurchases')->filter(function ($detail) {
            return $detail->freight > 0; // Only include freight values greater than 0
        });
        $totalQtys = $trndtl->pluck('corrugationpurchases')->where('vorcher_no')->sum('qty');
    @endphp
