            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">Ink Purchase</li>
                    </ol>
                </div>
                <h3 class="page-title">Ink Purchase</h3>
            </div>
        </div>
    </div>
    <!-- end page title -->
...
@endif
    ...
    <div class="row">
        ...
        <div class="col-6">
            <form action="{{ route('ink_purchase.reports') }}" method="GET" class="form-inline" id="search-form">
                <div class="row">
    ...
    <select name="account_id" class="form-control select2" data-toggle="select2">
        ...
        @endforeach
               ...
               <div class="form-group mt-3">
                                    ...
                                    <a class="btn btn-success" href="{{ route('ink_purchase.list') }}" role="button" onclick="return checkPermission()">Add New</a>
                                </div>
                </div>
            </form>
        </div>
            ...
            <table id="combined-data-table" class="table table-striped dt-responsive nowrap w-100">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $data->inkpurchases->items->item_code ?? 'N/A'}}</td>
                        <td>{{ $data->inkpurchases->qty ?? 'N/A'}}</td>
                        <td>{{ $data->inkpurchases->rate ?? 'N/A'}}</td>
                        <td>{{ $data->inkpurchases->amount ?? 'N/A'}}</td>
                        <td class="no-print">{{ $data->inkpurchases->freight ?? 'N/A'}}</td>
                        <td class="no-print">
                                                <input type="checkbox"
                        ...
                        <td class="no-print">
                            <form action="{{ route('ink_purchase.delete', ['id' => $data->id]) }}" method="POST" style="display:inline;" onclick="return checkPermissionDel()">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete(this)">Delete</button>
                            </form>

                            <a href="{{ route('ink_purchase.edit', ['v_no' => $data->v_no]) }}" class="btn btn-warning btn-sm" onclick="return checkPermissionEdit()">Edit</a>
                    <a href="{{ route('ink_purchase.editBoxboard', ['v_no' => $data->v_no]) }}" class="btn btn-primary btn-sm mt-1">Freight</a>
...
const freightContent = `
   @php
    ...
    $freightDetails = $trndtl->pluck('inkpurchases')->filter(function ($detail) {
        return $detail->freight > 0;
    });

    // Calculate total quantities
    $totalQtys = $trndtl->pluck('inkpurchases')->where('vorcher_no')->sum('qty');
@endphp
