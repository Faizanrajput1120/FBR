...
<head>
    ...
    <title>Multiple Sales Tax Invoices - {{ $invoices->first()->seller_business_name ?? 'SS ENTERPRISES' }}</title>
        ...
        <div class="container">
            <div class="company-info">
                ...
                <div class="company-tax">
                    NTN: {{ $invoice->seller_ntn_cnic ?? '9628363-1' }} | STRN: {{ $invoice->seller_strn ?? '3277876270368' }}
                </div>
            </div>
            ...
            <div class="header">
                ...
                <div class="date-bill">
                    <div><strong>Date:</strong> {{ \Carbon\Carbon::parse($invoice->invoice_date ?? now())->format('d/m/Y') }}</div>
                ...
                <div style="margin-top: 10px; padding: 0 8px;">
                    <table class="grand-total-table">
                        <tr>
                            ...
                            <td style="width: 12%; font-size: 9px;">{{ number_format($totalExclTax, 2) }}</td>
                            <td style="width: 10%;"></td>
                        </tr>
                        <tr>
                            <td>Total Sales Tax:</td>
                            <td style="font-size: 9px;">{{ number_format($totalSalesTax, 2) }}</td>
                            <td></td>
                        </tr>
                        @if($totalTransport > 0)
                            <tr>
                                <td>Transportation Charges:</td>
                                <td style="font-size: 9px;">{{ number_format($totalTransport, 2) }}</td>
                                <td></td>
                            </tr>
                        @endif
                        @if($totalFurtherTax > 0)
                            <tr>
                                <td>Further Tax:</td>
                                <td style="font-size: 9px;">{{ number_format($totalFurtherTax, 2) }}</td>
                                <td></td>
                            </tr>
                        @endif
                        <tr class="grand-row">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <div class="amount-words-value">{{ ssNumberToWords(intval($grandTotal)) }} Rupees Only</div>
            </div>

            <div class="qr-section">
                <div style="text-align: center;">
                    <div id="qrcode-{{ $invoice->id }}" class="qr-container"></div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            @foreach($invoices as $invoice)
                (function() {
                    ...
                    const qrElement = document.getElementById("qrcode-{{ $invoice->id }}");
