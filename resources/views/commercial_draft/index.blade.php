                    ...
                    <div class="mb-6">
                        <div class="flex flex-col sm:flex-row gap-4 items-center justify-between">
                            <div class="flex-1 max-w-lg">
                                <form method="GET" action="{{ route('commercial.drafts.index') }}" class="flex">
                            ...
                            @if($search)
                                <div class="text-sm text-gray-600">
                                    ...
                                    <a href="{{ route('commercial.drafts.index') }}" class="ml-2 text-blue-600 hover:text-blue-800">Clear</a>
                                </div>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($drafts as $draft)
                                <div class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                                    ...
                                    <div class="p-4">
                                        <div class="space-y-2">
                                            @if($draft->buyer_business_name)
                                                <div class="flex items-center text-sm">
                                                    <svg class="w-4 h-4 text-gray-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
                                                    </svg>
                                                    <span class="text-gray-600">{{ $draft->buyer_business_name }}</span>
                                                </div>
                                            @endif
                                            ...
                                            @if($draft->invoice_date)
                                                <div class="flex items-center text-sm">
                                                    ...
                                                    <span class="text-gray-600">{{ $draft->invoice_date->format('M d, Y') }}</span>
                                                </div>
                                            @endif
                                            ...
                                            @if($draft->items_count > 0)
                                                <div class="flex items-center text-sm">
                                                    ...
                                                    <span class="text-gray-600">{{ $draft->items_count }} item(s)</span>
                                                </div>
                                            @endif
                                            ...
                                            <div class="flex items-center text-sm">
                                                ...
                                                <span class="text-gray-600">Modified {{ $draft->formatted_last_modified_at }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="px-4 py-3 bg-gray-50 border-t border-gray-100 rounded-b-lg">
                                        <div class="flex items-center justify-between">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('commercial.drafts.edit', $draft) }}"
    ...
    <script>
        ...
        async function submitDraft(draftId) {
            ...
            try {
                ...
                const response = await fetch(`${API_BASE}/premiertax/commercial-drafts/${draftId}/submit`, {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': CSRF_TOKEN,
